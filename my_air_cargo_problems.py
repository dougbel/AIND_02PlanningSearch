...
class AirCargoProblem(Problem):
    ...
    def get_actions(self):
        ...
        def load_actions():
            ...
            # we have max 18 concrete load actions
    ...
    def actions(self, state: str) -> list:
        ...
        for action in self.actions_list:
            is_possible = True
            # first in every positive clause in preconditions
            for clause in action.precond_pos:
                if clause not in kb.clauses:
                    is_possible = False # if something fails then this actions is not possible
            # after check every negative clause in preconditions
    ...
    def result(self, state: str, action: Action):
        ...
        for fluent in old_state.pos:
            # check if fluent TRUE in old state continues TRUE as effect of an action
            if fluent not in action.effect_rem:
                new_state.pos.append(fluent)
        for fluent in old_state.neg:
            # check if fluent FALSE in old state continues FALSE as effect of an action
            if fluent not in action.effect_add:
                new_state.neg.append(fluent)

        for fluent in action.effect_add:
            # check if fluent that turns TRUE in new state is already there
            if fluent not in new_state.pos:
                new_state.pos.append(fluent)
        for fluent in action.effect_rem:
            # check if fluent that turns FALSE in new state is already there
            if fluent not in new_state.neg:
                new_state.neg.append(fluent)

        news = encode_state(new_state, self.state_map)
        return news
    ...
    def h_ignore_preconditions(self, node: Node):
        ...
        for clause in self.goal:
            if clause not in kb.clauses:
                #count how many differences are between actual state and the goal
                count+=1
...
def air_cargo_p3() -> AirCargoProblem:
    ...
    init = FluentState(pos, neg)
    goal = [expr('At(C1, JFK)'), expr('At(C2, SFO)'), expr('At(C3, JFK)'), expr('At(C4, SFO)'), ]
    return AirCargoProblem(cargos, planes, airports, init, goal)